/* global builderConfig, FusionEvents */
/* eslint no-shadow: 0 */
	...
	$( document ).ready( function() {
		...
		FusionPageBuilder.ModuleSettingsWpWidget = FusionPageBuilder.BaseWidgetSettingsView.extend( {
			...
			onInit: function() {
				...
				this.widgetData = builderConfig.widget_data;

				this.updateFusionWpWidget();
			},
			...
			formOptionChange: function( event ) {
				...
				if ( widget.isInvalid ) {
					paramName = this.formatName( widget.className, event.target.name );
				}
